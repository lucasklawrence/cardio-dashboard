...
import type { StepSample } from '../../types';
...
export function StepCountChart({ data, fig }: StepCountChartProps) {
  ...
  const config = useMemo<ChartConfiguration | null>(() => {
    ...
    const raw = data.map((d) => ({ date: d.date, value: d.count }));
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            ...
            callbacks: {
              label: (ctx) => `${Math.round(ctx.parsed?.y ?? 0).toLocaleString()} steps`,
            },
          },
        },
        scales: {
    ...
    <div className="section">
      <div className="section-header">
        ...
        <div className="section-header-controls">
          ...
          <GoalInput value={goal} onChange={setGoal} unit="steps" />
